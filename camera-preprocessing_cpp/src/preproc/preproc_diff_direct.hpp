#ifndef PREPROC_DIFF_DIRECT_H_
#define PREPROC_DIFF_DIRECT_H_

#include <cstdint>
#include "i_preproc_handler.hpp"

//Implements IPreprocHandler
class PreprocDiffDirect : public IPreprocHandler {
public:
   ...
   PreprocDiffDirect(uint8_t* input_buf, uint8_t* second_buf, uint8_t* grayscale_buf, uint8_t* second_grayscale_buf,
   ...
   void process() override {
      ...
      //Overwrite the previous frame buffer with the direct byte to byte difference of the current frame and previous frame.
      overwrite_previous_frame_with_direct_diff(current_frame_buf, previous_frame_buf, width, height, bpp, gate_value);
