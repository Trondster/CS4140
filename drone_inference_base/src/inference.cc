...
#include "inference.h"
#include "model_drone_detector_ptq_drone_int8.h"
...
extern "C" int drone_inference_init(void)
{
	...
	g_model = tflite::GetModel(tf_drone_detector_model_ptq);
	...
	for (int i = 0; i < g_interp->inputs_size(); i++) {
	...
	for (int i = 0; i < g_interp->outputs_size(); i++) {
